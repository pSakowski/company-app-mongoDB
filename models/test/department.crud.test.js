...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    it('should properly update one document with "updateOne" method', async () => {
      await Department.updateOne({ name: 'Department #1' }, { $set: { name: 'Updated Department #1' } });
      const updatedDepartment = await Department.findOne({ name: 'Updated Department #1' });
    ...
    it('should properly update one document with "save" method', async () => {
      ...
      const updatedDepartment = await Department.findOne({ name: 'Updated Department' });
      expect(updatedDepartment).to.not.be.null;
    });

    it('should properly update multiple documents with "updateMany" method', async () => {
      await Department.updateMany({}, { $set: { name: 'Updated Department' } });
      const departments = await Department.find();
      expect(departments[0].name).to.be.equal('Updated Department');
      expect(departments[1].name).to.be.equal('Updated Department');
    });
  });
